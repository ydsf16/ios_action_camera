...
- 连续自动对焦（不支持的镜头显示固定对焦），自动曝光最长 5 ms（1/200 秒），保留自动 ISO；视频与 IMU 使用系统时钟映射同步。
- 处理权限拒绝、空间不足、视频丢帧、传感器中断、相机中断、退后台收尾。

录制结束后自动排队生成 1080p 稳定视频，保留原片与原声音。素材预览可切换原片／稳定结果，并保存当前版本到相册。旧素材可以手动点击「生成稳定视频」。

Gyroflow 1.6.3 核心读取实际录制时钟映射、原始陀螺仪和逐帧完整内参，使用可调平滑与裁切；没有额外视觉时间补偿。当前采用 Metal（wgpu）GPU 重投影，Apple 框架负责解码、编码和音频封装。CPU 保留用于回归对照；GPU 初始化或渲染失败会停止导出并保留原片。当前 GPU 路径包含像素上传和读回。
