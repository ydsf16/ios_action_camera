...
- 0.8.1 重力水平锁定：适配层使用与 gyro 相同的 `host_sec ↔ video_sec` 映射，按重力自身时间戳送入核心。在原始后置相机像素坐标下，Gyroflow 重力元数据使用 `[-gy, -gx, -gz]`；此映射只应用一次，不改写 CSV。水平目标角再使用 `-displayRotationDegrees`，核心视频旋转保持 0°，导出保留原轨道显示矩阵。校验重力覆盖范围、采样缺口与向量长度；不将原始加速度当成重力。详见[轴向与像素验证](gravity-horizon-validation.md)。
